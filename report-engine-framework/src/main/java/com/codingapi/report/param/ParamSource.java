 ...
 *
 * <ul>
 *   <li>配置时只声明参数存在（名称、类型、是否必填、默认值），不给具体值
 *   <li>运行时由调用方传入：{@code render(reportId, {deptId: 5, year: 2026})}
 *   <li>所有 External 参数合起来 = 这张报表的"输入契约"，也是前端参数面板的渲染依据
 *   <li>例：部门报表的 deptId，月度报表的 year/month
 * </ul>
 *
 ...
 * <ul>
 *   <li>参数值由另一个格子的当前值提供
 *   <li>典型场景：用户在"部门"格子选了一个值，"部门名称"参数自动跟随更新
 *   <li>实现了"报表内部交互"而不需要外部重新传参
 * </ul>
 *
 ...
 * <ul>
 *   <li>值在设计时就确定了，不会变化
 *   <li>看起来多余（直接写死不行吗？），但好处是：给固定值一个有意义的名字， 让条件表达式更可读（{@code status = :ACTIVE_STATUS} 比 {@code
 *       status = 1} 清晰）
 * </ul>
 *
 * <h3>为什么用密封接口？</h3>
 *
 * <p>参数来源只有这三种（穷尽），密封接口 + record 实现让编译器帮助检查完整性： 如果新增了一种来源，所有 switch/instanceof 的地方都会编译报错，强制处理新情况。
 *
 * <h3>与 Value 表达式的关系</h3>
 *
 * <p>ParamSource 决定参数"从哪取值"（配置视角）， {@link com.codingapi.report.expression.Value}
 * 表达式决定单元格/条件/汇总"计算什么值"（使用视角）。 ParamSource.External 参数 → 被 {@code Value.ParamValue} 引用 → 运行时求值。
 */
