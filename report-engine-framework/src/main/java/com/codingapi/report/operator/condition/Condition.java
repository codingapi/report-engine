 ...
 * <h3>使用位置</h3>
 * <ul>
 *   <li>{@code CellBinding.getConditions()} — 格子级别的过滤，在渲染阶段生效，只影响该格子的数据范围</li>
 *   <li>{@link com.codingapi.report.data.dataset.Query#getFilters()} — 查询级别的过滤，
 *       在数据提取阶段生效，影响整个数据集的行数</li>
 * </ul>
 *
 * <h3>左右值都是 {@link Value} 表达式</h3>
 * <p>条件两端统一用值表达式树，运行期由 {@code ExpressionEngine} 求值后交给
 * {@link ConditionPredicate} 比较。这样左值不限于裸字段（可写 {@code base+bonus}），
 * 右值天然支持字面量 / 报表参数 / 循环字段：
 * <pre>
