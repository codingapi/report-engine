 ...
 * <p>原本比较逻辑集中在一个 {@code switch(operator)} 里——每加一个算子就要改这个方法，
 * 且未实现的算子被 {@code default} 静默放行（导致过滤失效）。改为按算子拆成独立策略后：
 * <ul>
 *   <li><b>开闭原则</b>：新增算子 = 新增一个实现类并注册，不改动已有代码</li>
 *   <li><b>显式失败</b>：没有策略支持的算子由 {@link ConditionPredicates} 抛异常，而非静默放行</li>
 * </ul>
 * 这与数据提取层的 {@code DataExtractor}（{@code supports()} + 注册列表）是同一套扩展范式。
 *
 * <h3>输入约定</h3>
 * <p>右值已由 {@code ParamContext} 解析为实际值（字面量/参数/循环字段都已求值），
 * 所以本接口只接收两个裸值，不感知参数体系——保持纯粹、易测、无外部依赖。
 *
 * @see CompareOperator
