 ...
 *
 * <p>原本聚合逻辑集中在一个 {@code switch(aggregation)} 里——每加一种聚合就要改这个方法。 改为按聚合方式拆成独立策略后，新增聚合（如
 * COUNT_TRUE/COUNT_FALSE）= 新增一个实现类并注册， 不改动已有代码。这与 {@code DataExtractor} / {@code ValueFunction}
 * 是同一套扩展范式： {@code supports(name)} 选中 + {@link Aggregators} 注册表分发。
 *
 * <h3>元信息（借鉴 {@link com.codingapi.report.expression.function.ValueFunction}）</h3>
 *
 * <p>每个算子通过 {@link #meta()} 自描述（名称/显示名/参数/说明），供前端表达式构建器枚举与展示， 前端不再硬编码聚合清单，由注册表统一分发。
 *
 * @see Aggregators
