 ...
 * <p>物理字段名通常是英文/拼音（如 {@code xm}、{@code base_sal}），不适合直接展示给用户。
 * {@code alias} 用于属性面板的字段选择列表、生成的报表表头等面向用户的场景。
 * <b>绑定引用只用 name，不用 alias</b>——改别名不破坏任何绑定。
 *
 * <h3>{@link #primaryKey} 的作用</h3>
 * <p>标记主键字段，用于两个场景：
 * <ul>
 *   <li>自动扫描 Relationship 时，外键指向的目标必须是主键</li>
 *   <li>UI 层面可以在字段列表里标识主键列，辅助用户理解数据结构</li>
 * </ul>
 */
@Data
@Builder
