 ...
 * <h3>在架构中的位置</h3>
 *
 * <p>Query 正好压在"提取 / 加工"的边界上——本系统的数据计算全部在 Java 完成， 连接只负责提取原始数据：
 *
 * <pre>
 ...
 *
 * <ul>
 *   <li>{@link #groupBy} 非空 → 按分组<b>去重</b>迭代。 例如"按部门循环"，迭代集合 = 去重后的部门列表，每个部门一次
 *   <li>{@link #groupBy} 为空 → <b>逐行</b>迭代。 例如"按员工循环"，每个员工一条记录，每人一张薪资条
 * </ul>
 *
 * <h3>为什么不直接用 SQL 字符串？</h3>
 *
 * <p>Query 是声明式的（描述"要什么"），不是过程式的（不写"怎么取"）。 这样同一个 Query 可以适配不同 DataSourceType：DB 类型拼成 SQL，CSV
 * 类型在内存过滤， API 类型转为请求参数。
 */
@Data
@Builder
public class Query {
     ...
     * 本数据集上的过滤条件。
     *
     * <p>条件的左右值都是 {@link com.codingapi.report.expression.Value} 表达式， 可以是字面量（{@code
     * Value.Literal}）、报表参数（{@code Value.ParamValue}）、 循环字段（{@code
     * Value.LoopFieldValue}）等，使过滤范围能被运行时动态控制。
     */
     ...
     * 分组字段列表（本数据集内的字段名）。
     *
     * <p>非空时，循环按分组去重迭代（如按"部门"分组 → 每个部门一次）； 为空时，循环逐行迭代（如每个员工一次）。
     */
