 ...
 * <p>关系本质上是"两个 Dataset 之间"的事，而两个 Dataset 可能来自不同的 DataSource
 * （如人事库的员工表 JOIN 薪资库的薪资表）。如果关系挂在某个 DataSource 上，
 * 跨源关系就无处安放。因此关系独立存在：
 * <ul>
 *   <li>通用关系 → 挂在 {@link DataModel#getRelationships()}，所有引用该模型的报表共享</li>
 *   <li>报表专有关系 → 挂在 {@link com.codingapi.report.render.Report#getExtraRelationships()}，
 *       仅本报表可见</li>
 * </ul>
 *
 * <h3>它不是 SQL JOIN</h3>
 ...
 * <p>由 {@link #origin} 区分：
 * <ul>
 *   <li>{@code AUTO}：从数据库外键元数据自动扫描，适合单库内的标准关联</li>
 *   <li>{@code MANUAL}：用户在界面上手动拖线连接，支持跨源、跨库的任意关联</li>
 * </ul>
 */
@Data
@Builder
