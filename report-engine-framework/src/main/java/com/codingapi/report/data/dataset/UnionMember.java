 ...
 * <ul>
 *   <li>A 部门员工表列名：{@code name, gender, age}</li>
 *   <li>B 部门员工表列名：{@code xm, xb, nl}（用拼音缩写）</li>
 * </ul>
 ...
 * </pre>
 *
 * <p>与 SQL UNION 的区别：这里的成员可以来自不同连接类型（如 DB 表 + CSV 文件），
 * SQL UNION 只能处理同一数据库内的表。
 *
 * @param datasetId 成员数据集 id，指向 {@link Dataset#getId()}
 * @param mapping   统一列名 → 成员字段名。key 是 UNION 数据集的列名（即
 *                  {@link Dataset#getFields()} 中的 name），value 是该成员数据集的实际字段名
 */
