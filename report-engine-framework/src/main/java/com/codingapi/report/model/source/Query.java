...
/**
 * 查询定义：从<b>一个数据集</b>提取数据的意图。主要用作 {@link com.codingapi.report.model.grid.LoopBlock}
 * 的驱动源（决定循环迭代的范围和顺序）。
 *
 * <h3>在架构中的位置</h3>
 ...
 *       而是放在 {@link com.codingapi.report.model.grid.FieldCell#getConditions()} 里）</li>
 * </ul>
 *
 * <h3>分组与迭代的关系</h3>
 * <ul>
...
public class Query {
    ...
    /**
     * 本数据集上的过滤条件。
     * <p>条件右值可以是字面量（{@link com.codingapi.report.model.param.ValueRef.Literal}）、
     * 报表参数（{@link com.codingapi.report.model.param.ValueRef.Param}）或
     * 循环字段（{@link com.codingapi.report.model.param.ValueRef.LoopField}），
     * 使过滤范围能被运行时动态控制。
     */
    private List<com.codingapi.report.model.grid.Condition> filters;
    /**
     * 分组字段列表（本数据集内的字段名）。
