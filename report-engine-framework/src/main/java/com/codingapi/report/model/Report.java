 ...
 * 但共享底层的数据集与关系。这是"数据定义"与"报表布局"的关注点分离：
 * <pre>
 ...
 * <p>静态内容（纯标题文字、样式、边框、合并、行列尺寸）全部在 {@code templateId} 指向的
 * Univer 工作簿里，Report 不存储任何样式信息。{@code cellBindings} / {@code loopBlocks}
 * 只是语义覆盖层——告诉渲染引擎"这个格子要取什么数、怎么扩展"。
 * 最终序列化时，cellBindings 写入 {@code Cell.props}，loopBlocks 写入
 * {@code Sheet.loopBlocks}——这两个槽位 Univer 结构里已经预留。
 *
...
public class Report {
     ...
     * <p>承载全部静态文本、样式、边框、合并、行列尺寸。cellBindings 按坐标覆盖其上，
     * 不影响模板本身的静态内容。
     */
     ...
     * <p>作为 {@link DataModel#getRelationships()} 的补充，渲染时两者合并使用。
     * 这是例外而非常态——大多数关系应该放在共享的 DataModel 里。
     ...
     * <p>循环块的迭代值不是参数，通过 {@link com.codingapi.report.model.param.ValueRef.LoopField} 直接引用。
     */
     ...
     * <ul>
     *   <li>{@link com.codingapi.report.model.grid.TextCell} — 文本 + 参数占位（如"${year}年度报表"）</li>
     *   <li>{@link com.codingapi.report.model.grid.FieldCell} — 数据字段绑定 + 扩展 + 分组 + 条件</li>
     * </ul>
     ...
     * 每个循环块绑定一个 {@link com.codingapi.report.model.source.Query}（数据集 + 过滤 + 分组），
     * 决定迭代的范围和顺序。块内格子可通过 {@link com.codingapi.report.model.param.ValueRef.LoopField}
     * 引用当前迭代行的字段。
     */
     ...
     * <p>例如"按单位分组"的员工报表，可以在每个单位结束后插入一行"XX单位小计"，
     * 全表末尾插入一行"总计"。行位置随数据量自适应。
     */
