 ...
 *
 * <p>{@link Value.FunctionCall} 的实际行为由这里提供——"支持哪些函数"是预先约定（注册）的。 与 {@code Aggregator}/{@code
 * ConditionPredicate} 同一套范式：{@code supports(name)} 选中 + {@link Functions}
 * 注册表分发。新增函数（round/concat/if…）= 实现本接口并登记，零改动接入。
 */
