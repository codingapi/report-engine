 ...
 *
 * <h3>定位：全模型统一的取值机制</h3>
 * <p>报表里所有"动态值"——单元格内容、过滤条件的左右值、小计行的标签/聚合——本质都是"对运行时上下文
 * 求一个值"。本接口把这件事收成一棵可 JSON 序列化的表达式树，由 {@link ExpressionEngine} 求值。
 * 这取代了原先散落的多套机制（{@code TextCell.template} 的 ${} 正则、{@code FieldCell.aggregation}、
 * {@code ValueRef} 等）。
 *
 * <h3>节点类型（预先约定，密封穷尽）</h3>
 * <ul>
 *   <li>{@link Literal} — 字面量/纯文本</li>
 *   <li>{@link FieldValue} — 读当前行的某字段</li>
 ...
 *
 * <h3>求值是策略机制</h3>
 * <p>每种节点对应一个 {@link ValueEvaluator} 策略，由 {@link ExpressionEngine} 按 {@code supports()}
 * 选中分发——与 {@code DataExtractor}/{@code Aggregator}/{@code ConditionPredicate} 同一套范式。
 */
public sealed interface Value
        permits Value.Literal, Value.FieldValue, Value.ParamValue, Value.LoopFieldValue,
        Value.NameRef, Value.Template, Value.Aggregate, Value.FunctionCall {
     ...
     * <p>文本插值 {@code ${name}} 编译成它：作者写名字时不必区分"这是参数还是循环字段"，
     * 由 {@code ParamContext.lookup} 按作用域就近解析。明确来源时用 {@link ParamValue}/{@link LoopFieldValue}。
     */
