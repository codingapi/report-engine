 ...
 * <p>例：{@code round(3.14159, 2)} → {@code 3.14}；{@code round(3.5)} → {@code 4.0}。
 * 非数值原样返回字符串，null 返回 null，digits 缺省为 0。
 */
public class RoundFunction implements ValueFunction {
    ...
    public FunctionMeta meta() {
        return new FunctionMeta("round", "四舍五入",
                List.of("数值", "小数位"),
                "对数值四舍五入到指定小数位，例：round(value, 2)");
    }
