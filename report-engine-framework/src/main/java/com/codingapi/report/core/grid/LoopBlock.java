 ...
 *
 * <ul>
 *   <li>循环范围能被参数控制：{@code filters} 里可以写 {@code dept_id = :deptId}， 运行时只循环指定部门的员工
 *   <li>{@code groupBy} 决定"逐行迭代"（每人一次）还是"按分组去重迭代"（每部门一次）
 * </ul>
 *
 * <h3>循环字段免登记</h3>
 *
 * <p>循环把"当前迭代行"发布进作用域，块内格子通过 {@code Value.LoopFieldValue}{@code (loopId, field)} 直接引用驱动数据集的字段，
 * 无需预先在 {@link com.codingapi.report.core.Report#getParameters()} 里登记。
 *
 ...
 *
 * <ul>
 *   <li><b>静态 JOIN</b>：两个数据集在提取阶段一次性合并，适合两个表结构固定、关系明确
 *   <li><b>循环驱动</b>：父迭代传键 → 子查询逐次执行，适合子源异构（甚至可以是 API）， 或者需要"模板重复呈现"的场景（如每人一张独立的薪资条）
 * </ul>
 *
...
public class LoopBlock {
     ...
     *
     * <p>查询结果有多少行（或多少分组），循环就执行多少次。 每次迭代，当前行的字段被发布到 ParamContext 的循环作用域中。
     */
