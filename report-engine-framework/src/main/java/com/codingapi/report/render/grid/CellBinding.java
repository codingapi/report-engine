 ...
 *
 * <ul>
 *   <li><b>值层</b>（{@link #value}）：这个格子最终显示<b>什么值</b>——纯文本、字段、聚合、格式化… 全部统一为一棵 {@link Value} 表达式树，由
 *       {@code ExpressionEngine} 求值。 值层只关心"算出一个值"，不碰样式/边框/合并。
 *   <li><b>控制层</b>（expansion / expandMode / mergeRepeated / parentCell / conditions）：
 *       这个值<b>怎么在格子上铺开</b>——纵向/横向扩展、分组去重、相邻合并、多级分组父格链、数据过滤。
 * </ul>
 *
 ...
 *
 * <p>"文本格子 vs 字段格子"的区别其实只是<b>值的来源不同</b>，把它做成子类型等于用类型当值的开关。 现在区别完全落在 {@link #value} 这棵表达式树的根节点上：纯文本
 * = {@code Template}/{@code Literal}， 字段 = {@code FieldValue}，聚合 = {@code
 * Aggregate}……控制层属性对所有格子一视同仁（按需配置）。
 *
 * <h3>渲染两层处理</h3>
 *
 * <pre>
...
public class CellBinding {
    ...
    /** 父格：多级分组的对齐参照格。串成父格链实现逐级嵌套；null 表示顶层。 父格方向（左/上）由父格自身的 {@link #expansion} 推断。 */
     ...
     * 独立纵向带（显式配置，默认 false）。
     *
     * <p>默认情况下，同一数据集的纵向列会被对齐成<b>一条带</b>（一行一记录，列共享行）。 置为 {@code true}
     * 时，本列<b>不与同源列对齐</b>，而是从它自己的声明行起<b>独立向下展开</b>， 形成"交错/并列"排版。
     *
     * <p>代价：独立带与其它列之间<b>无法再做跨列聚合（parentCell 引用别列）、跨列汇总行、主从合并</b>，
     ...
     *
     * <p>开启后，预览态下该格渲染为可点击（蓝色链接样式），用户点击可查看聚合/汇总计算的明细数据。 仅对聚合格有意义；普通字段/文本格开启无效果。
     *
