...
class OperatorsTest {
    ...
    private static Relationship rel(JoinType jt) {
        return Relationship.builder().id("rel")
                .left(new FieldRef("d_edu", "emp_id"))
                .right(new FieldRef("d_emp", "id"))
                .joinType(jt).origin(RelationOrigin.MANUAL).build();
    }
    ...
    void inner_shouldKeepOnlyMatched() {
        ...
        // 一对多：emp1 产出两行
        assertEquals(2, out.getRows().stream().filter(r -> "张三".equals(r.get("d_emp.name"))).count());
    }
    ...
    void left_shouldKeepAllLeftWithNullFill() {
        ...
        Map<String, Object> emp3Row = out.getRows().stream()
                .filter(r -> "王五".equals(r.get("d_emp.name"))).findFirst().orElseThrow();
        assertNull(emp3Row.get("d_edu.degree"), "emp3 无学历，degree 应为 null");
        assertNull(emp3Row.get("d_edu.emp_id"), "emp3 无学历，emp_id 应为 null");
        // 列含双方
        assertTrue(out.getColumns().containsAll(List.of("d_emp.id", "d_emp.name", "d_edu.emp_id", "d_edu.degree")));
    }
    ...
    void right_shouldKeepAllRightWithNullFill() {
        ...
        Map<String, Object> edu9Row = out.getRows().stream()
                .filter(r -> "博士".equals(r.get("d_edu.degree"))).findFirst().orElseThrow();
    ...
    void full_shouldKeepBothSides() {
        ...
        // 两侧无匹配行都存在
        assertTrue(out.getRows().stream().anyMatch(r -> "王五".equals(r.get("d_emp.name")) && r.get("d_edu.degree") == null),
                "emp3 保留且学历为 null");
        assertTrue(out.getRows().stream().anyMatch(r -> "博士".equals(r.get("d_edu.degree")) && r.get("d_emp.name") == null),
                "emp9 保留且员工为 null");
    }

    @Test
    void nullJoinType_shouldDefaultToInner() {
        Relationship noType = Relationship.builder().id("rel")
                .left(new FieldRef("d_edu", "emp_id"))
                .right(new FieldRef("d_emp", "id"))
                .origin(RelationOrigin.MANUAL).build();
