...
首个试点：`packages/report-engine/src/components/property-panel/drill-editor.test.tsx`。以下是落地中确认的、与 vitest/jest 直觉不同的点，供后续组件测试直接套用。
...
export default defineConfig({
  ...
  setupFiles: ['./src/test/setup.ts'], // 顶层！非 test.setupFiles
  globals: true,
});
```

**全局 API 与 vitest 不同**：rstest 没有 `vi`。`describe/it/test/expect/beforeAll/afterEach` 等开启 `globals: true` 后注入全局；mock 函数用全局 **`rs.fn()`**（或 `rstest.fn()`），`rs.spyOn()` 取代 `vi.spyOn()`。用全局 API 时在文件顶部加 `/// <reference types="@rstest/core/globals" />` 拿类型。
...
> 说明：试点组件 `DrillEditor` 纯 props 驱动、无接口依赖，故本批 MSW 仅搭骨架（`src/test/handlers.ts` + `msw-server.ts` + setup 接线），当前用例未触发请求；待接入有接口依赖的组件时再补常态 handler 与 `server.use()` 异态覆盖。
  ...
  projects: [
    {
      ...
      exclude: ['src/**/*.{style,visual}.{test,spec}.{ts,tsx}'],
    },
    {
      name: 'browser',
      extends: withRslibConfig(),
      ...
      globals: true,
      include: ['src/**/*.style.{test,spec}.{ts,tsx}'],
    },
  ],
});
```
