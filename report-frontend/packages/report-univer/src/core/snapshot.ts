...
function parseFont(raw: Record<string, unknown>): ExcelFont | undefined {
  ...
  let has = false;
  if (raw.ff) {
    font.family = raw.ff as string;
    has = true;
  }
  if (raw.fs) {
    font.size = raw.fs as number;
    has = true;
  }
  if (raw.bl === 1) {
    font.bold = true;
...
function parseBorders(bd: unknown): ExcelBorders | undefined {
  ...
  let has = false;
  const t = parseBorder(raw.t);
  if (t) {
    borders.top = t;
    has = true;
  }
  const r = parseBorder(raw.r);
  if (r) {
    borders.right = r;
    has = true;
  }
  const b = parseBorder(raw.b);
  if (b) {
    borders.bottom = b;
    has = true;
  }
  const l = parseBorder(raw.l);
  if (l) {
    borders.left = l;
    has = true;
  }
...
function parsePadding(pd: unknown): ExcelStyle['padding'] | undefined {
  ...
  let has = false;
  if (raw.t != null) {
    result.top = raw.t as number;
    has = true;
  }
  if (raw.r != null) {
    result.right = raw.r as number;
    has = true;
  }
  if (raw.b != null) {
    result.bottom = raw.b as number;
    has = true;
  }
  if (raw.l != null) {
    result.left = raw.l as number;
    has = true;
  }
...
export function extractSnapshot<TCellProp = unknown, TLoopProp = unknown>(
  ...
  const rawSheets = (snapshot as Record<string, unknown>).sheets as
    | Record<string, Record<string, unknown>>
    | undefined;
  const globalStyles = (snapshot as Record<string, unknown>).styles as
    | Record<string, Record<string, unknown>>
    | undefined;
  ...
  for (const [sheetId, sheetData] of Object.entries(rawSheets)) {
    ...
    // 单元格数据
    const cellData = sheetData.cellData as
    ...
    if (cellData) {
      for (const [rowStr, rowData] of Object.entries(cellData)) {
        ...
        for (const [colStr, rawCell] of Object.entries(rowData)) {
          ...
          const value =
            rawCell.v != null && rawCell.v !== ''
              ? (rawCell.v as string | number | boolean)
              : richTextValue;

          const formula =
            typeof rawCell.f === 'string' && rawCell.f.startsWith('=')
              ? rawCell.f.slice(1)
              : undefined;
