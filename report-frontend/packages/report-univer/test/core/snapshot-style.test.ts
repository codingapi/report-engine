...
describe('parseFont', () => {
    test('完整字体属性全部映射', () => {
        const style = styleOf({
            ff: 'Arial', fs: 14, bl: 1, it: 1,
            ul: { s: 1 }, st: { s: 1 }, cl: { rgb: '#FF0000' },
        });
        expect(style?.font).toEqual({
            family: 'Arial', size: 14, bold: true, italic: true,
            underline: true, strikethrough: true, color: '#FF0000',
        });
    });
...
describe('parseStyle: 整体输出形态', () => {
    test('所有样式项齐备时整体结构完整', () => {
        const style = styleOf({
            ff: 'Arial', fs: 12, bl: 1, cl: { rgb: '#AA0000' },
            ht: 2, vt: 2, tb: 3, tr: { a: 90 },
            bg: { rgb: '#BBFFCC' }, n: { pattern: '0.00' },
            pd: { t: 2, r: 2, b: 2, l: 2 },
            bd: { t: { s: 1, cl: { rgb: '#000000' } } },
        });
        expect(style).toEqual({
            font: { family: 'Arial', size: 12, bold: true, color: '#AA0000' },
            align: 'center', valign: 'middle', wrap: true, rotation: 90,
            fill: '#BBFFCC', numberFormat: '0.00',
