...
interface SheetMock { getRange: ReturnType<typeof rs.fn>; highlightRanges: ReturnType<typeof rs.fn> }
...
describe('createHighlightManager', () => {
    ...
    test('sync 移除已删除的块：旧区域被 dispose、不再应用', () => {
        ...
        const { api } = makeUniver({ s1: sheet });
        const hm = createHighlightManager(api);
        hm.sync({ b1: { id: 'b1', sheetId: 's1', startRow: 0, startColumn: 0, endRow: 0, endColumn: 0 } });
    ...
    test('sheet 不存在时不抛错', () => {
        ...
        const hm = createHighlightManager(api);
        expect(() =>
            hm.sync({ b1: { id: 'b1', sheetId: 'missing', startRow: 0, startColumn: 0, endRow: 0, endColumn: 0 } }),
        ).not.toThrow();
        hm.dispose();
    });
