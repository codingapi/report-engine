...
describe('toValueDTO — 递归映射', () => {
    ...
    test('Aggregate 递归映射 operand', () => {
        ...
        expect(toValueDTO(v)).toEqual({
            type: 'Aggregate', aggregation: 'SUM',
            payload: undefined, funcName: undefined, args: undefined, parts: undefined,
            operand: { type: 'FieldValue', payload: 'ds.salary' },
        });
    });
    ...
    test('FunctionCall 映射 args 数组', () => {
        const v: ReportValue = {
            ...
            args: [{ type: 'FieldValue', payload: 'd.x' }, { type: 'Literal', payload: 'fmt' }],
        };
...
describe('toBindingDTO — 剥离设计态 displayText', () => {
    test('剥离 displayText，保留权威字段与 preview/drill', () => {
        const binding: CellBinding = {
            ...
            parentCell: null,
            conditions: [
                { id: 'c1', left: { type: 'FieldValue', payload: 'ds.age' }, operator: 'GT', right: { type: 'Literal', payload: '18' } },
            ],
            independent: true,
            preview: '${员工表.姓名}',
            drillEnabled: true,
    ...
    test('independent 缺省为 false', () => {
        const binding: CellBinding = {
            ...
            mergeRepeated: false, parentCell: null, conditions: [],
        };
    ...
    test('right 为 null 时保持 null', () => {
        const binding: CellBinding = {
            ...
            value: { type: 'Literal', payload: 'x' },
            expansion: 'NONE', expandMode: 'GROUP',
            mergeRepeated: false, parentCell: null,
            conditions: [{ id: 'c1', left: { type: 'FieldValue', payload: 'd.x' }, operator: 'IS_NULL', right: null }],
        };
