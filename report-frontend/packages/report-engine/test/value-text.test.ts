...
describe('parseTemplate — 归约规则', () => {
  ...
  test('文本 + 洞混合 → Template', () => {
    ...
    expect(v.parts?.filter((p) => p.kind === 'text').map((p) => p.text)).toEqual(['合计 ', ' 元']);
...
describe('valueDisplayText — 别名友好展示', () => {
  ...
  test('FieldValue → ${数据集别名.字段别名}（取值类整体包 ${}）', () => {
    expect(valueDisplayText({ type: 'FieldValue', payload: 'ds.name' }, datasets)).toBe(
      '${员工表.姓名}',
    );
  });

  test('NameRef/ParamValue → ${参数别名}（缺失回退参数名）', () => {
    expect(valueDisplayText({ type: 'NameRef', payload: 'startDate' }, datasets, [], params)).toBe(
      '${开始日期}',
    );
    expect(valueDisplayText({ type: 'NameRef', payload: 'unknown' }, datasets, [], params)).toBe(
      '${unknown}',
    );
  });
