...
export function styleToCss(style?: ExcelStyle): React.CSSProperties {
  ...
  if (f) {
    ...
    const deco = [f.underline ? 'underline' : '', f.strikethrough ? 'line-through' : ''].filter(
      Boolean,
    );
  ...
  if (bd) {
    const t = borderToCss(bd.top);
    if (t) css.borderTop = t;
    const r = borderToCss(bd.right);
    if (r) css.borderRight = r;
    const b = borderToCss(bd.bottom);
    if (b) css.borderBottom = b;
    const l = borderToCss(bd.left);
...
function renderCellContent(cell?: ExcelCell): React.ReactNode {
  ...
  if (rt && rt.segments && rt.segments.length) {
    return rt.segments.map((seg, i) => (
      <span key={i} style={styleToCss({ font: seg.style })}>
        {seg.text}
      </span>
    ));
  }
...
export const SheetTable: React.FC<{
  ...
  const { cellMap, anchorSpan, covered, maxRow, maxCol, rowH, rowHidden, colW, colHidden } = model;

  return (
    <table
      style={{ borderCollapse: 'collapse', tableLayout: 'fixed', color: 'rgba(0, 0, 0, 0.88)' }}
    >
      <colgroup>
        {Array.from({ length: maxCol + 1 }, (_, c) => (
          <col
            key={c}
            style={{
              width: `${colHidden.has(c) ? 0 : (colW.get(c) ?? sheet.defaultColumnWidth)}px`,
            }}
          />
        ))}
      </colgroup>
