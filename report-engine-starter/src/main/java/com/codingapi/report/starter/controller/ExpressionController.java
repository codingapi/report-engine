 ...
 * <p>
 * 聚合由 {@link Aggregators} 注册表自描述（每个 {@code Aggregator} 提供 {@link FunctionMeta}）；
 * 函数由 {@link Functions} 注册表自描述。两边都是策略+注册表范式，新增聚合/函数只需登记一行。
 */
@RestController
...
public class ExpressionController {
    ...
    public SingleResponse<ExpressionCatalog> functions() {
        ...
        List<FunctionMeta> aggregations = Aggregators.list().stream()
                .filter(m -> !"NONE".equals(m.name()))
                .toList();
