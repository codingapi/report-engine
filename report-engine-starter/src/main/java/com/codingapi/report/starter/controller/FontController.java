 ...
 *
 * <p>仅当 classpath 中存在 Spring Web（RestController）时自动注册。
 */
@RestController
...
public class FontController {
     ...
     *
     * <p>每个字体族仅返回第一个文件（目录已按文件名排序，第一个即首选变体）。 不返回内置字体（Univer 已自带，重复添加会抛异常，且 @font-face 会覆盖系统字体）。
     */
    @GetMapping("/list")
    public MultiResponse<FontItem> listFonts() {
        // 收集内置字体的 family 名称（排除与内置同名的自定义字体，避免 @font-face 覆盖系统字体）
        Set<String> builtinFamilies =
                fontRegistry.getBuiltinFontCatalog().stream()
        ...
        return MultiResponse.of(
                fontRegistry.getCustomFontCatalog().stream()
