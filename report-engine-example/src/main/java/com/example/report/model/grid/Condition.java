 ...
 *       只影响该格子的数据范围</li>
 *   <li>{@link com.example.report.model.source.Query#getFilters()} — 查询级别的过滤，
 *       在数据提取阶段生效，影响整个数据集的行数</li>
 * </ul>
 *
 * <h3>右值 {@link #value} 是关键的扩展点</h3>
 * <p>条件右值不是只能填固定值——通过 {@link com.example.report.model.param.ValueRef}
 * 密封接口，支持三种来源：
 * <pre>
 *   1. 字面量（Literal）：
...
public class Condition {
    ...
    /**
     * 右值：字面量、参数引用或循环字段引用。
     * <p>通过 {@link com.example.report.model.param.ValueRef} 密封接口的三种实现，
     * 让过滤条件能静态配置、动态参数化、循环上下文感知。
     * <p>IS_NULL/IS_NOT_NULL 算子时可为 null。
     */
    private com.example.report.model.param.ValueRef value;
}
