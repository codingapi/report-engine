 ...
 *
 * <p>对应 ReportScenarioTest 中的 8 个场景。示例报表使用<strong>写死的稳定 id</strong> （如 {@code
 * example-simple-list}），保证重启后 id 不变，前端引用不会失效。 前端通过 {@code GET /api/report/configs}
 * 统一列表获取（示例与用户报表同表）， 点击即导航到 {@code /engine?id=xxx} 打开对应报表。
 *
 * <p>配置用 {@link ReportConfigBuilder} 链式构造（产物为强类型 {@link ReportConfig}）。
 */
@Slf4j
@Component
public class ReportTemplateSeeder {
    ...
    private void seedSimpleList() {
        save(
                "example-simple-list",
                new ReportConfigBuilder("商品清单报表")
    ...
    private void seedMergedList() {
        save(
                "example-merged-list",
                new ReportConfigBuilder("销售明细报表")
    ...
    private void seedStatistics() {
        save(
                "example-statistics",
                new ReportConfigBuilder("人员统计报表")
    ...
    private void seedMasterDetail() {
        save(
                ...
                new ReportConfigBuilder("员工学历信息表")
                        ...
                        .binding(
                                2,
                                5,
                                fieldValue("emp_education", "graduate_time"),
                                "VERTICAL",
                                "LIST")
                        .template(
                                8,
    ...
    private void seedSubtotal() {
        save(
                "example-subtotal",
                new ReportConfigBuilder("薪资统计报表")
                        ...
                        .summary(
                                ...
                                0,
                                3,
                                new FieldRefDTO("salary_detail", "unit"),
                                List.of(
    ...
    private void seedPayslipLoop() {
        ...
        ConditionDTO loopCondition =
                new ConditionDTO(
                        "c1",
                        fieldValue("salaries", "emp_id"),
                        "EQ",
                        new ValueDTO(
                                "LoopFieldValue", "loop_emp.id", null, null, null, null, null));

        LoopBlockDTO loopBlock =
                new LoopBlockDTO(
                        "loop_emp",
                        "员工循环",
                        SHEET,
        ...
        save(
                "example-payslip-loop",
                new ReportConfigBuilder("薪资条报表")
                        ...
                        .bindingWithConditions(
                                2,
                                0,
                                fieldValue("salaries", "total"),
                                "NONE",
                                "LIST",
                                loopCondition)
                        .bindingWithConditions(
                                2, 1, fieldValue("salaries", "base"), "NONE", "LIST", loopCondition)
                        .bindingWithConditions(
                                2,
                                2,
                                fieldValue("salaries", "bonus"),
                                "NONE",
                                "LIST",
                                loopCondition)
                        .loopBlock(loopBlock)
    ...
    private void seedIndependentBands() {
        save(
                "example-independent-bands",
                new ReportConfigBuilder("员工商品并列报表")
    ...
    private void seedIndependentBandsWithSummary() {
        ...
        save(
                "example-independent-bands-summary",
                new ReportConfigBuilder("员工商品并列汇总报表")
                        .binding(0, 0, literal("姓名"), "NONE", "LIST")
                        .binding(0, 1, literal("单位"), "NONE", "LIST")
                        .binding(0, 2, literal("商品名"), "NONE", "LIST")
                        .binding(0, 3, literal("价格"), "NONE", "LIST")
                        ...
                        .summary(
                                ...
                                1,
                                null,
                                List.of(labelCell(0, "员工合计"), aggCell(1, "staff.name", "COUNT")))
                        .summary(
                                2,
                                2,
                                3,
                                null,
                                List.of(labelCell(2, "商品合计"), aggCell(3, "products.price", "SUM")))
                        .template(
                                8,
                                6,
                                cell(0, 0, "姓名"),
    ...
    private void seedCrossTab() {
        ...
        save(
                "example-cross-tab",
                new ReportConfigBuilder("区域季度销售交叉表")
    ...
    private void seedHorizontalSummary() {
        ...
        String ds = "products";
        save(
                "example-horizontal-summary",
                new ReportConfigBuilder("商品横向汇总表")
