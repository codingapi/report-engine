 ...
 *
 * <p>产物是强类型 {@link
 * ReportConfig}（name/dataModelId/cellBindings/loopBlocks/summaries/params/template）。 值工厂为静态方法（返回
 * {@link ValueDTO}），便于在 seed 方法内直接组装。
 */
public class ReportConfigBuilder {
    ...
    public ReportConfigBuilder binding(
            int row, int col, ValueDTO value, String expansion, String expandMode) {
    ...
    public ReportConfigBuilder bindingWithConditions(
            int row,
            int col,
            ValueDTO value,
            String expansion,
    ...
    private BindingDTO makeBinding(
            int row,
            int col,
            ValueDTO value,
            String expansion,
    ...
    public ReportConfigBuilder summary(
            int row,
            int fromColumn,
            int toColumn,
            FieldRefDTO groupBy,
    ...
    public ReportConfigBuilder summaryColumn(
            int col, int fromRow, int toRow, FieldRefDTO groupBy, List<SummaryCellDTO> cells) {
        summaries.add(
                new SummaryRowDTO(
                        "sum-" + System.nanoTime(),
                        "HORIZONTAL",
        ...
        while (i < text.length()) {
            ...
            parts.add(
                    new PartDTO(
                            "hole",
                            null,
                            new ValueDTO("NameRef", name, null, null, null, null, null)));
            i = end + 1;
        }
